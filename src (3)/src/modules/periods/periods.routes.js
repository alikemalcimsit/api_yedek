...
// PATCH /periods/current-status
router.patch(
  '/current-status',
  domainMiddleware, authenticateToken, slidingSession,
  periodsController.updateCurrentStatusByUserPatientId
);

// PUT /periods/bulk-status-range
router.put(
  '/bulk-status-range', 
  domainMiddleware, authenticateToken, slidingSession,
  periodsController.bulkUptadePeriodsWithStoredProcedure
);
